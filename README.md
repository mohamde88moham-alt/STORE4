...
- Add products with size, color, and quantity selection
- Persistent cart using localStorage
- Real-time cart updates and notifications

✅ **Product Selection**
- Interactive size selector (S, M, L, XL)
...
- **Sync to all devices:** Go to **GitHub Sync** in the admin sidebar. Enter your GitHub Personal Access Token (with `repo` scope), repo owner, and repo name. After that, every save (products, content, design, orders, etc.) is pushed to `data/store-data.json` in your repo. The live site loads this file so changes appear on desktop and mobile immediately. Without GitHub Sync, data is stored only in the browser (per device).
